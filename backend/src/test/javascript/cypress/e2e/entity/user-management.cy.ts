...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Damon.Dickinson76', email: 'Alberta_Schiller@yahoo.com' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Robyn_Wolff32');
      cy.get(`[data-cy="login"]`).should('have.value', 'Robyn_Wolff32');

      cy.get(`[data-cy="firstName"]`).type('Elisa');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Elisa');

      cy.get(`[data-cy="lastName"]`).type('Borer');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Borer');

      cy.get(`[data-cy="email"]`).type('Joy.Medhurst@yahoo.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Joy.Medhurst@yahoo.com');
